...
class FunctionalMetrics(PipelineStage, LoggingMixin, TimingMixin):
    ...
    def compute_quality_scores(self, df: pd.DataFrame) -> Dict[str, float]:
    ...
    def compute_size_and_score(self, df: pd.DataFrame) -> tuple[int, Dict[str, float]]:
        """Compute size and quality scores for a dataframe subset."""
        df = df.drop_duplicates(subset=["conversation_id", "model"])
        size = len(df)
        quality_scores = self.compute_quality_scores(df)
    ...
    def compute_cluster_metrics(self, df: pd.DataFrame, clusters: List[str], models: List[str]) -> Dict[str, Any]:
    ...
    def _compute_model_cluster_scores(self, df: pd.DataFrame, cluster_names: List[str], model_names: List[str]) -> Dict[str, Dict[str, Dict[str, Any]]]:
        """Compute metrics for each model-cluster combination."""
        model_cluster_scores = {}
        for model in model_names:
            model_cluster_scores[model] = {
                cluster: self.compute_cluster_metrics(df, cluster, [model])
                for cluster in cluster_names
            }
        return model_cluster_scores

    def _compute_cluster_scores(self, df: pd.DataFrame, cluster_names: List[str], model_names: List[str]) -> Dict[str, Dict[str, Any]]:
        """Compute metrics for each cluster across all models."""
        return {
            cluster: self.compute_cluster_metrics(df, cluster, list(model_names))
            for cluster in cluster_names
        }

    def _compute_model_scores(self, df: pd.DataFrame, cluster_names: List[str], model_names: List[str]) -> Dict[str, Dict[str, Any]]:
        """Compute metrics for each model across all clusters."""
        return {
            model: self.compute_cluster_metrics(df, list(cluster_names), [model])
            for model in model_names
        }
    ...
    def _generate_plots(self, model_cluster_scores, cluster_scores, model_scores):
        ...
        self.log(f"✅ Generated {len(plot_files)} plot files for {num_quality_metrics} quality metrics")
        self.log(f"📁 Plots saved to {plots_dir}")
        
        if self.log_to_wandb and wandb.run:
            self.log("📊 Plots automatically logged to wandb under 'Plots/' namespace")
