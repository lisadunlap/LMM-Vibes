...
export function ColumnSelector({ 
  ...
  const renderChips = (selectedCols: string[], color: 'primary' | 'secondary' = 'primary') => {
    ...
    <Box sx={{ p: 3, border: '1px solid #e0e0e0', borderRadius: 2, mb: 2 }}>
      ...
      <FormControl fullWidth sx={{ mb: 2 }}>
        ...
        <Select
          ...
          renderValue={(selected) => (
            <Box sx={{ display: 'flex', flexWrap: 'wrap' }}>
              {renderChips(selected, 'secondary')}
            </Box>
          )}
        >
        ...
        <FormHelperText>
          Optional: Select columns containing numeric scores or score dictionaries. 
          Multiple columns will be combined, single dictionary columns will be flattened.
        </FormHelperText>
      </FormControl>
