#!/usr/bin/env python3
"""
Run the LMM-Vibes pipeline on the WildBench dataset.

This is a convenience script for running the full pipeline on the wildbench dataset
...
def main():
    """Main function for wildbench dataset processing."""
    parser = argparse.ArgumentParser(description="Run LMM-Vibes pipeline on WildBench dataset")
    
    # Output directory
    parser.add_argument("--output_dir", type=str, 
                        default="results/wildbench_full_pipeline",
                        help="Output directory for results (default: results/wildbench_full_pipeline)")
    ...
    if not os.path.exists(data_path):
        print(f"Error: WildBench dataset not found at {data_path}")
        print("Please make sure the dataset is available.")
        return
    
    print("="*60)
    print("WILDBENCH DATASET PIPELINE")
    print("="*60)
    ...
    # Run pipeline with wildbench-optimized parameters
    ...
    print(f"\n🎉 WildBench pipeline completed! Results saved to: {args.output_dir}")
